...
外部サービスである「drug-navigator」と連携し、医薬品の代替薬検索機能を提供する。

#### 3.4.2. 連携方法

-   **トップメニューからのアクセス:**
    `index.html`の機能選択メニューに「代替薬ナビゲーター」の項目を追加。クリックすると、新しいタブで「drug-navigator」のトップページ (`https://drug-navigator.pages.dev/`) が開かれる。
-   **医薬品検索結果からのアクセス:**
    `search.html`の医薬品検索結果テーブルに表示される医薬品名をクリックすると表示されるドロップダウンメニューに「代替薬ナビゲーター」の項目を追加。この項目をクリックすると、新しいタブで「drug-navigator」に遷移し、該当医薬品名がクエリパラメータ (`?medicineName=`) として渡され、検索入力欄に自動入力された状態で検索が開始される。

#### 3.4.3. 注意事項

-   「drug-navigator」は生成AI（Gemini 2.5 Flash Light）を利用しており、その出力情報の正確性、完全性、最新性は保証されない。利用者は、所属団体の生成AI利用規約等を遵守し、自身の責任において利用する必要がある。詳細は利用規約を参照。

### 3.5. 花粉飛散リアルタイム監視 (`pollen-app/`)
