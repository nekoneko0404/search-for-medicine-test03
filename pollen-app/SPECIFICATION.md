...
*   **表示数制限と優先順位**:
    *   ズームレベルに応じて最大表示マーカー数 (`maxMarkers`) を設定 (30〜1000)。
    *   **グリッドサンプリング**: 画面をグリッド状に分割し、各グリッドから均等に地点を選出することで、特定地域へのマーカー集中を防ぐ。
    *   **優先順位**:
        1.  都市レベル (Level 1 > 2 > 3)
        2.  画面中心 (またはサイドパネルを考慮した視覚的中心) からの距離が近い順
*   **色分け (5段階)**:
