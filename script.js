...
function processExcelData(data) {
    try {
        ...
        const mappedData = dataRowsAsStrings.map((row, index) => {
            ...
            return {
                'productName':          row !== undefined ? String(row) : '',
                'ingredientName':       row !== undefined ? String(row) : '',
                'manufacturer':         row !== undefined ? String(row) : '',
                'shipmentStatus':       row !== undefined ? String(row) : '',
                'reasonForLimitation':  row !== undefined ? String(row) : '',
                'resolutionProspect':   row !== undefined ? String(row) : '',
                'expectedDate':         (numberRow !== undefined ? String(numberRow) : '') || (row !== undefined ? String(row) : ''),
                'shipmentVolumeStatus': row !== undefined ? String(row) : '',
                'yjCode':               /^[0-9A-Z]{12}$/.test(row) ? String(row) : '', // YJコードの形式検証
                'standard':             row !== undefined ? String(row) : '',
                'isGeneric':            row !== undefined ? String(row) : '',
                'isBasicDrug':          row !== undefined ? String(row) : '',
                'updateDateSerial':     (numberRow !== undefined ? String(numberRow) : '') || (row !== undefined ? String(row) : '')
            };
        });
...
function renderTable(data) {
    ...
    displayResults.forEach((item, index) => {
        ...
        if (!item.yjCode) {
            ...
            const isValidYjCode = /^[0-9A-Z]{12}$/.test(item.yjCode);
            const pmdaLinkUrl = isValidYjCode ? `https://www.pmda.go.jp/PmdaSearch/rdSearch/02/${item.yjCode}?user=1` : '#';
            const yjCodeLinkUrl = isValidYjCode ? `./yjcode.html?yjcode=${item.yjCode}` : '#';
            const hiyariLinkUrl = isValidYjCode ? `./hiyari_app/index.html?drugName=${drugNameForHiyari}` : '#';
