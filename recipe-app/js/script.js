...
function renderSymptoms() {
    symptomsContainer.innerHTML = SYMPTOMS.map(s => `
        <label class="cursor-pointer group">
            ...
            <span class="inline-block px-4 py-2 bg-white border border-gray-200 rounded-full text-gray-700 
                hover:bg-orange-50 hover:border-orange-300 transition-all select-none text-sm
                peer-checked:bg-orange-500 peer-checked:text-white peer-checked:border-orange-500 peer-checked:shadow-md peer-checked:scale-105 transform">
                ${s.label}
            </span>
        </label>
    `).join('');
...
function renderRecipes(data) {
    ...
    data.recipes.forEach((recipe, index) => {
        ...
        div.innerHTML = `
            <div class="bg-orange-100 p-4 border-b border-orange-200 flex justify-between items-start">
                <div>
                ...
                <div class="bg-white p-2 rounded-full shadow-sm text-2xl">
                    🥘
                </div>
            </div>
            ...
            <div class="p-5">
                <div class="mb-4">
                    <h4 class="font-bold text-gray-700 mb-2 border-l-4 border-green-500 pl-2">健康ポイント</h4>
                    <p class="text-sm text-green-700 bg-green-50 p-3 rounded-lg">
                        ${escapeHtml(recipe.health_point)}
                    </p>
                </div>
