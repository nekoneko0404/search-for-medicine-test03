...
*   **県別トレンドグラフ**: 地図上のエリアまたは詳細リストの県名をクリックすると、地図と入れ替わりでその県の週次推移グラフ（折れ線）を表示します。
    *   流行レベル（注意報・警報・流行入り）に応じて線の色が変化します（インフルエンザ、手足口病など多数の疾患に対応）。
    *   **比較機能強化**: Y軸のスケールを全国最大値に統一し、都道府県間の感染レベルを視覚的に比較しやすくしました。
    *   **視認性向上**: 今年のデータを強調（ポイントサイズ調整、過去データの色調整）し、トレンドを把握しやすくしました。
    *   **インタラクティブな凡例**: グラフの凡例（年）をクリックすると、その年のデータを強調表示し、他の年をグレーアウトして比較しやすくします。
    *   **ARIの扱い**: 急性呼吸器感染症 (ARI) は週次推移データのグラフ表示に対応していません。
*   **その他の感染症**: 主要3疾患以外の感染症（RSウイルス、感染性胃腸炎など、流行性耳下腺炎を含む）の週次推移を一覧表示します。
    *   都道府県ごとの表示切り替えが可能です。
...
*   **Backend**: Google Apps Script (GAS), Google Sheets, Google Drive
    *   GAS: 定期的にIDWRからCSVをダウンロードし、スプレッドシートに書き込む。また、最新の週報データ（CSV）をGoogleドライブの「過去週報」フォルダに自動保存（アーカイブ）する機能を持つ。`doGet` エンドポイントでCSVデータを提供する。
