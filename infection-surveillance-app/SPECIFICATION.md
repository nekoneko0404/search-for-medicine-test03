...
### 3.3. データ出力・API提供
- GAS Web Appの `doGet` 関数は、リクエストパラメータ `type` に応じてスプレッドシートのデータを各形式（JSON/CSV）で返す。
- **エンドポイントの種類**:
  - `type=current`: 当年の最新データ（サマリー、地図データ、当年の履歴）をJSON形式で提供。初期読み込みに使用。
  - `type=history`: 過去数年分の全アーカイブデータをJSON形式で提供。バックグラウンド読み込みまたはグラフ表示時に使用。
  - `type=combined`: `current` と `history` を統合したデータを提供（旧バージョンとの互換性用）。
  - その他、`teiten`, `ari`, `tougai` などの各シートをCSV形式で取得可能。
...
### 4.2. 機能
- **データ取得・キャッシュ**:
  - 初回起動時、`type=current` を取得して地図とサマリーを即座に表示する。
  - アーカイブデータ（`type=history`）はバックグラウンドで非同期に取得し、完了後にグラフ表示を可能にする。
  - `localforage` を利用して、当年データ（1時間有効）とアーカイブデータ（24時間有効）を分離してキャッシュする。
  - メニュー画面（ルートの `index.html`）でのデータ先読みは行わず、各アプリ起動時にのみ読み込む。
