import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css              */let l,h,v,c,y,f,_,b,p,u,x,g;const $="https://recipe-worker.neko-neko-0404.workers.dev",w=[{label:"血圧が高め",value:"血圧"},{label:"血糖値が気になる",value:"血糖値"},{label:"腎臓をいたわりたい",value:"腎臓"},{label:"肝臓をいたわりたい",value:"肝臓"},{label:"体重が気になる",value:"減量"},{label:"尿酸値が高め",value:"尿酸値"},{label:"骨密度が気になる",value:"骨強化"},{label:"筋力をつけたい",value:"筋力アップ"},{label:"認知機能を維持したい",value:"脳活性化"},{label:"中性脂肪が気になる",value:"中性脂肪"},{label:"コレステロールが高め",value:"コレステロール"},{label:"風邪気味",value:"免疫力"},{label:"肩こりがひどい",value:"肩こり解消"},{label:"冷え性",value:"血行促進"},{label:"疲れが取れない",value:"疲労回復"},{label:"便秘気味",value:"腸内環境"},{label:"貧血気味",value:"鉄分補給"},{label:"肌荒れ",value:"美肌"}];function L(){l=document.getElementById("recipe-form"),h=document.getElementById("symptoms-container"),v=document.getElementById("api-key-input-container"),c=document.getElementById("api-key"),y=document.getElementById("advanced-settings-toggle"),f=document.getElementById("advanced-settings"),_=document.getElementById("result-section"),b=document.getElementById("loading"),p=document.getElementById("recipe-cards"),u=document.getElementById("save-api-key"),x=document.getElementById("save-key-warning"),g=document.getElementById("save-form-state"),h?A():console.error("Symptoms container not found!"),l&&(T(),q(),D(),H()),console.log("Recipe App Init Completed")}window.debugInit=L;function A(){h.innerHTML=w.map(t=>`<label class="cursor-pointer group">
        ...
        </label>`).join("")}function T(){y&&f&&y.addEventListener("click",()=>{f.classList.toggle("hidden");const s=y.querySelector("i");f.classList.contains("hidden")?s.classList.remove("rotate-180"):s.classList.add("rotate-180")}),u&&u.addEventListener("change",s=>{s.target.checked?(x.classList.remove("hidden"),c.value&&localStorage.setItem("recipe_app_user_key",c.value)):(x.classList.add("hidden"),localStorage.removeItem("recipe_app_user_key"))}),c&&c.addEventListener("input",s=>{u.checked&&localStorage.setItem("recipe_app_user_key",s.target.value)});const t=document.getElementsByName("api_option");Array.from(t).forEach(s=>{s.addEventListener("change",a=>{const n=a.target.value;localStorage.setItem("recipe_app_provider",n),n==="system"?v.classList.add("hidden"):v.classList.remove("hidden")})}),l.addEventListener("submit",C);const r=document.getElementById("print-btn");r&&r.addEventListener("click",()=>{document.querySelectorAll("#recipe-cards details").forEach(o=>o.setAttribute("open","true"));const a=document.getElementById("print-header"),n=document.querySelectorAll(".recipe-card");if(document.querySelectorAll(".cloned-header").forEach(o=>o.remove()),a&&n.length>0)for(let o=1;o<n.length;o++){const i=a.cloneNode(!0);i.classList.add("cloned-header"),i.id="",i.querySelectorAll("img").forEach(m=>{m.loading="eager";const d=m.src;m.src="",m.src=d}),n[o].parentNode.insertBefore(i,n[o])}window.print(),setTimeout(()=>{clonedHeaders.forEach(o=>o.remove())},100)});const e=document.getElementById("copy-prompt-btn");e&&e.addEventListener("click",B)}function S(){const t=new FormData(l),r=Array.from(t.getAll("symptoms")),e=t.get("other_symptom");e&&e.trim()!==""&&r.push(e.trim());const s=Array.from(t.getAll("ingredient")).filter(i=>i.trim()!==""),a=Array.from(t.getAll("excluded_ingredient")).filter(i=>i.trim()!==""),n=t.get("cuisine"),o=t.get("time");return{symptoms:r,ingredients:s,excludedIngredients:a,cuisine:n,time:o}}async function B(){const t=S(),r=t.symptoms.length>0?t.symptoms.join("、"):"特になし",e=t.ingredients.length>0?t.ingredients.join("、"):"おまかせ",s=t.excludedIngredients.length>0?t.excludedIngredients.join("、"):"なし",n=`あなたは管理栄養士かつ一流のシェフです。
...
- **ingredients**: 各食材の情報をオブジェクトの配列で記載してください。
  ...
  - substitute: 代替食材（日本で入手困難な本格食材を使用する場合のみ記載）。例: "レモン汁(大さじ1) + ショウガ薄切り"`;try{await navigator.clipboard.writeText(n);const o=document.getElementById("copy-prompt-btn"),i=o.innerHTML;o.innerHTML='<i class="fas fa-check"></i> コピーしました！',o.classList.remove("bg-blue-50","text-blue-700","border-blue-200"),o.classList.add("bg-green-50","text-green-700","border-green-200"),setTimeout(()=>{o.innerHTML=i,o.classList.add("bg-blue-50","text-blue-700","border-blue-200"),o.classList.remove("bg-green-50","text-green-700","border-green-200")},3e3)}catch(o){console.error("Failed to copy keys: ",o),alert("クリップボードへのコピーに失敗しました。")}}document.addEventListener("DOMContentLoaded",L);async function C(t){t.preventDefault(),console.log("Form submitted!"),_.classList.remove("hidden"),p.innerHTML="",b.classList.remove("hidden"),_.scrollIntoView({behavior:"smooth"});const e=new FormData(l).get("api_option");let s=null;if((e==="openai"||e==="gemini")&&(s=c.value.trim()),(e==="openai"||e==="gemini")&&!s){k("APIキーが入力されていません。設定を確認するか、「おまかせ」を選択してください。"),b.classList.add("hidden");return}const{symptoms:a,ingredients:n,excludedIngredients:o,cuisine:i,time:E}=S(),m={symptoms:a,ingredients:n,excludedIngredients:o,cuisine:i,time:E,provider:e==="openai"?"openai":"gemini"};try{const d=await M(m,s);P(d)}catch(d){console.error("Error:",d),k(d.message,d.status,e)}finally{b.classList.add("hidden")}}function q(){const t=localStorage.getItem("recipe_app_user_key"),r=localStorage.getItem("recipe_app_provider");if(r){const e=document.querySelector(`input[name="api_option"][value="${r}"]`);e&&(e.checked=!0,r!=="system"&&v.classList.remove("hidden"))}t&&(c.value=t,u&&(u.checked=!0,x.classList.remove("hidden")))}function I(){if(g&&!g.checked)return;S();const t=new FormData(l),r={symptoms:t.getAll("symptoms"),other_symptom:t.get("other_symptom"),ingredients:t.getAll("ingredient"),excluded_ingredients:t.getAll("excluded_ingredient"),excluded_ingredients:t.getAll("excluded_ingredient"),cuisine:t.get("cuisine"),time:t.get("time")};localStorage.setItem("recipe_app_form_state",JSON.stringify(r))}function D(){if(!(localStorage.getItem("recipe_app_enable_history")!=="false"))return;const r=localStorage.getItem("recipe_app_form_state");if(r)try{const e=JSON.parse(r);if(e.symptoms&&document.querySelectorAll('input[name="symptoms"]').forEach(a=>{a.checked=e.symptoms.includes(a.value)}),e.other_symptom){const s=document.querySelector('input[name="other_symptom"]');s&&(s.value=e.other_symptom)}if(e.ingredients){const s=document.querySelectorAll('input[name="ingredient"]');e.ingredients.forEach((a,n)=>{s[n]&&(s[n].value=a)})}if(e.excluded_ingredients){const s=document.querySelectorAll('input[name="excluded_ingredient"]');e.excluded_ingredients.forEach((a,n)=>{s[n]&&(s[n].value=a)})}if(e.cuisine){const s=document.querySelector(`input[name="cuisine"][value="${e.cuisine}"]`);s&&(s.checked=!0)}if(e.time){const s=document.querySelector(`input[name="time"][value="${e.time}"]`);s&&(s.checked=!0)}}catch(e){console.error("Failed to restore form state:",e)}}function H(){if(g){const e=localStorage.getItem("recipe_app_enable_history")!=="false";g.checked=e,g.addEventListener("change",s=>{const a=s.target.checked;localStorage.setItem("recipe_app_enable_history",a),a?I():localStorage.removeItem("recipe_app_form_state")})}let t;const r=()=>{clearTimeout(t),t=setTimeout(I,500)};l.addEventListener("change",r),l.addEventListener("input",r)}async function M(t,r){const e={"Content-Type":"application/json"};r&&(e["X-User-Key"]=r);try{const s=await fetch($,{method:"POST",headers:e,body:JSON.stringify(t)}),a=await s.text();let n;try{n=JSON.parse(a)}catch{throw new Error(a||`Server Error: ${s.status}`)}if(!s.ok){const o=n.error||`Server Error: ${s.status} ${s.statusText}`;console.error("Fetch Error Details:",s.status,o,n),location.hostname==="localhost"||location.hostname.includes("pages.dev");const i=new Error(o);throw i.status=s.status,i}return n}catch(s){throw s}}function P(t){if(!t.recipes||t.recipes.length===0){p.innerHTML='<p class="text-center text-gray-500">レシピが見つかりませんでした。</p>';return}const r=e=>String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;");if(t.message){const e=document.createElement("div");e.className="col-span-1 md:col-span-2 lg:col-span-3 bg-orange-50 p-4 rounded-xl border border-orange-200 text-orange-800 mb-4 fade-in",e.innerHTML=`<i class="fas fa-comment-medical mr-2"></i><strong>AIからのアドバイス:</strong> ${r(t.message)}`,p.appendChild(e)}t.recipes.forEach((e,s)=>{const a=document.createElement("div");a.className="recipe-card fade-in-up w-full",a.style.animationDelay=`${s*.2}s`,a.innerHTML=`
            <details class="group">
